...
def Clean_Mysql_DB(my_host,my_port,my_user,my_passwd,my_db,sql1,sql2,sql3,sql4,sql5,sql6_mys,sql7,sql8,sql9,sql10,sql11):
    ...
    print("开始清除mysql环境的数据......")
    logging.info("开始清除mysql环境的数据......")
    ...
    try:
        ...
        #先执行sql5 判断'中国工商银行'是否为直联，不直联就执行sql6_mys 变成直联。
        cursor.execute(sql5)
        number = cursor.fetchone()
        #print(number)
        if number is None:
            cursor.execute(sql6_mys)
...
if __name__ == "__main__":
    ...
    Clean_Mysql_DB(my_host, my_port, my_user, my_passwd, my_db,sql1,sql2,sql3,sql4,sql5,sql6_mys,sql7,sql8,sql9,sql10,sql11)
